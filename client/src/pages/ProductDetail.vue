...
<style scoped>
.product-detail-page { padding: 4rem 0 8rem; }
.back-link {
     display: flex;
      align-items: center;
       gap: 8px; 
...
.product-layout { display: grid; grid-template-columns: 1fr 1fr; gap: 6rem; }
.main-image { 
    border-radius: 32px; 
...
.thumbnail-grid { display: flex; gap: 1.25rem; }
.thumb { width: 90px; height: 90px; border-radius: 16px; overflow: hidden; cursor: pointer; border: 2.5px solid transparent; transition: 0.3s; background: white; }
.thumb img { width: 100%; height: 100%; object-fit: cover; }
.thumb.active { border-color: var(--accent-blue); transform: translateY(-5px); }
...
.inventory-status { display: flex;
     align-items: center; gap: 12px;
      margin-top: 1.5rem; font-size: 1rem; 
      font-weight: 600;
       color: #ef4444; 
    }
.inventory-status.ok { color: #10b981; }
...
.p-description { padding: 3rem; margin-bottom: 3.5rem; }
.p-description h3 { font-size: 1.5rem; margin-bottom: 1.5rem; font-weight: 800; }
.p-description p {
...
.p-actions { display: flex; gap: 2rem; align-items: center; margin-bottom: 5rem; }
.qty-control { 
    display: flex; align-items: center;
     gap: 2rem; padding: 0.75rem 1.5rem;
      border-radius: 100px; height: 64px; 
      background: #f1f5f9; 
    }
.qty-control button { 
    background: none;
    ...
    cursor: pointer; font-weight: 700;
     ...
     border-radius: 10px;
      transition: 0.2s; }
.qty-control button:hover { background: white; color: var(--accent-blue); }
.qty-control span { 
    font-weight: 900; font-family: 'Outfit';
     width: 40px; text-align: center; 
     font-size: 1.25rem; }

.p-actions .btn-premium { flex: 1; height: 64px; justify-content: center; font-size: 1.15rem; }

.p-trust-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; }
.trust-item { 
    display: flex; align-items: center; 
    gap: 15px; font-size: 0.95rem;
     color: var(--text-secondary);
      font-weight: 600; }
.trust-item svg { color: var(--accent-blue); }

@media (max-width: 1024px) {
    .product-layout { grid-template-columns: 1fr; gap: 4rem; }
}
</style>
