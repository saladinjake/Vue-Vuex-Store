...
.cart-header { margin-bottom: 4rem; text-align: center; }
.cart-header h1 { font-size: 3.5rem; margin-bottom: 1rem; letter-spacing: -2px; }
...
.cart-main { display: flex; flex-direction: column; gap: 1.5rem; }
...
.p-cat { 
  ...
  display: block; }
.item-info h3 { font-size: 1.4rem; margin-bottom: 0.5rem; color: var(--text-primary); }
.p-vendor { 
...
.remove-btn:hover { opacity: 1; background: #fef2f2; transform: scale(1.1); }
...
.cart-summary h3 { margin-bottom: 2.5rem; font-size: 1.8rem; font-weight: 900; }

.summary-lines { display: flex;
...
.checkout-btn { width: 100%; justify-content: center; height: 64px; font-size: 1.1rem; }
...
.empty-icon { color: #f1f5f9; margin-bottom: 2.5rem; }
.empty-cart h2 { 
  font-size: 2.5rem; 
  margin-bottom: 1rem;
   font-weight: 900; }
.empty-cart p { 
  color: var(--text-secondary);
   margin-bottom: 3.5rem; font-size: 1.1rem; }
...
@media (max-width: 768px) {
  ...
  .item-controls { flex-direction: column; gap: 2rem; width: 100%; }
